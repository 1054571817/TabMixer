...
class TabularDataLoader:
    def __init__(self, path_data, noise_std=0.0):
        self.df = pd.read_csv(path_data, index_col=[0])
        self.y = self.df["y"]
        self.df = self.df.drop(columns=["y", "test_set_fold"])
        self.num_tab = len(self.df.columns)
        self.col_names = list(self.df.columns)
        if noise_std != 0.0:
            noise = np.random.normal(0, noise_std, self.df.shape)
            self.df = self.df + noise

    def load_pid_data(self, pid):
        return self.df.loc[pid].to_numpy().astype(np.float32), self.y.loc[pid]
