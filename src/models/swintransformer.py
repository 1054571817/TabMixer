...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(2, 7, 7), shift_size=(0, 0, 0),
                 ...
                 act_layer=nn.GELU, norm_layer=GroupNorm, use_checkpoint=False):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_concat:
            self.final = nn.Linear(in_features=768 + tab_concat_dim, out_features=1)
        else:
            self.final = nn.Linear(in_features=768, out_features=num_classes)

        if pretrained is not None and pretrained != "":
            print(f"Initializing weights of Swin Transformer from {pretrained}")
            state_dict = torch.load(pretrained)["state_dict"]
            n_state_dict = {}
            for k, v in state_dict.items():
                if "patch_embed" in k:
                    continue
                n_state_dict[k.replace("backbone.", "")] = v
            self.load_state_dict(n_state_dict, strict=False)

        if last_layer_bias is not None:
            print(f"Initializing last layer bias to: {last_layer_bias}")
            nn.init.constant_(self.final.bias, last_layer_bias)
        self._freeze_stages()
    ...
    def init_weights(self, pretrained="../checkpoints/swin_tiny_patch244_window877_kinetics400_1k.pth"):
    ...
    def forward(self, x, tab=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == 0 and self.tab2 is not None:
                x = self.tab2(x, tab)
            if i == 1 and self.tab3 is not None:
                x = self.tab3(x, tab)
            if i == 2 and self.tab4 is not None:
                x = self.tab4(x, tab)
            if i == 3 and self.tab5 is not None:
                x = self.tab5(x, tab)
...
if __name__ == "__main__":
    ...
    model = SwinTransformer3D(in_chans=1, tab_concat_dim=None)
