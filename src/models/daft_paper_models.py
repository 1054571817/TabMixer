...
if __name__ == "__main__":
    from modules import TabAttention, TabMixer
else:
    from models.modules import TabAttention, TabMixer
...
class INTERACTIVE_MODEL(BaseModel):
    """
    ...
    def __init__(
        ...
        if last_layer_bias is not None:
            ...
            nn.init.constant_(self.fc.bias, last_layer_bias)
...
class TabAttentionBlock(FilmBase):
    # Block for ZeCatNet
    def __init__(
            self,
            in_channels: int,
        ...
        self.module = TabAttention(channel_dim=16, frame_dim=2, hw_size=(24, 24), tab_dim=ndim_non_img)

    def rescale_features(self, feature_map, x_aux):
        # B, 16,2,25,25
        return self.module(feature_map, x_aux)


class TABATTENTION_MODEL(BaseModel):
    def __init__(
            self,
            in_channels: int,
            n_outputs: int,
        ...
        self.block2 = ResBlock(n_basefilters, 2 * n_basefilters, bn_momentum=bn_momentum, stride=2)  # 16
        self.block3 = ResBlock(2 * n_basefilters, 4 * n_basefilters, bn_momentum=bn_momentum, stride=2)  # 8
        self.blockX = TabAttentionBlock(4 * n_basefilters, 8 * n_basefilters, bn_momentum=bn_momentum,
                                        **filmblock_args)  # 4
        self.global_pool = nn.AdaptiveAvgPool3d(1)
        self.fc = nn.Linear(8 * n_basefilters, n_outputs)
        if last_layer_bias is not None:
...
class TabMixerBlock(FilmBase):
    ...
    def __init__(
        ...
        self.module = TabMixer(channel_dim=16, frame_dim=2, hw_size=(24, 24), tab_dim=ndim_non_img)
...
class TABMIXER_MODEL(BaseModel):
    def __init__(
            self,
            in_channels: int,
            n_outputs: int,
        ...
        self.block2 = ResBlock(n_basefilters, 2 * n_basefilters, bn_momentum=bn_momentum, stride=2)  # 16
        self.block3 = ResBlock(2 * n_basefilters, 4 * n_basefilters, bn_momentum=bn_momentum, stride=2)  # 8
        self.blockX = TabMixerBlock(4 * n_basefilters, 8 * n_basefilters, bn_momentum=bn_momentum,
                                        **filmblock_args)  # 4
        self.global_pool = nn.AdaptiveAvgPool3d(1)
        self.fc = nn.Linear(8 * n_basefilters, n_outputs)
        if last_layer_bias is not None:
...
if __name__ == "__main__":
    ...
    y = torch.randn(2, 32)
    model = TABMIXER_MODEL(in_channels=1, n_outputs=1, filmblock_args={"ndim_non_img": 32})
