...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers, stem, num_classes=1, zero_init_residual=False, head=False,
                 msha=False, n_frames_last_layer=2, input_last_layer=(7, 7), last_layer_bias=None,
                 tabular_config=None, tab_concat_dim=None):
        ...
        if not head:
            ...
            if self.use_concat:
                self.fc = nn.Linear((512 * block.expansion) + tab_concat_dim, num_classes)
            else:
    ...
    def forward(self, x, y=None):
        ...
        if self.tab3 is not None:
            x = self.tab3(x, y)

        x = self.layer3(x, None)

        if self.tab4 is not None:
            x = self.tab4(x, y)

        x = self.layer4(x, y)

        if self.tab5 is not None:
            x = self.tab5(x, y)
...
def r3d_18(pretrained=False, progress=True, msha=False, n_frames=16, input_size=(224, 224), last_layer_bias=None,
           tabular_config=None, tab_concat_dim=None,
           **kwargs):
    ...
    return _video_resnet('r3d_18', pretrained, progress, block=BasicBlock, conv_makers=[Conv3DSimple] * 4,
                         layers=[2, 2, 2, 2], stem=BasicStem, msha=msha, n_frames_last_layer=n_frames_last_layer,
                         input_last_layer=input_last_layer, last_layer_bias=last_layer_bias,
                         tabular_config=tabular_config, tab_concat_dim=tab_concat_dim, **kwargs)


if __name__ == "__main__":
    x = torch.randn(2, 1, 16, 224, 224)
    y = torch.randn(2, 1, 6)
    model = r3d_18(tab_concat_dim=None)
